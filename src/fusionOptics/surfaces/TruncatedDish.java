...
/** Spherical Surface - Part of the surface of sphere, truncated on both sides.
 */
public class TruncatedDish extends Surface {
	...
	public TruncatedDish(String name, double centre[], double dishCentreNormal[], double up[], double radiusOfCurvature, double width, double rimRadius, Interface iface) {
		this(name, centre, dishCentreNormal, up, radiusOfCurvature, width, rimRadius, null, null, iface);
	}
	/**
	 * @param name
	 ...
	 * @param interfaceType
	 */
	public TruncatedDish(String name, double centre[], double dishCentreNormal[], double up[], double radiusOfCurvature, double width, double rimRadius,
	...
	public boolean findEarlierIntersection(RaySegment ray, Intersection hit) {
		...
		double[] side = Util.reNorm(Util.cross(up, dishNormal));
		if( hit1){ 
			if(hit2){	//both hit, return the earliest
				if(s1 < s2){
					if(s1 < ray.length){
						if(FastMath.abs(Util.dot(Util.minus(X1, centre), side)) < width/2){
					...
					if(s2 < ray.length){						
						if(FastMath.abs(Util.dot(Util.minus(X2, centre), side)) < width/2){
	...
	public boolean surfaceGeometryEquals(Surface obj) {
		TruncatedDish other = ((TruncatedDish)obj);
